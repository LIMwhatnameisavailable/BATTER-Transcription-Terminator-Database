...
SITE_ASSET_VERSION = "20260816-user-search-v2"
...
<main class="page-shell edge-demo" data-accession-demo data-default-accession="GCF_000739105.1">
  ...
  <section class="data-intro-grid" aria-label="{esc('About BTED data / 关于 BTED 数据')}">
    <article><span class="intro-number">01</span><div><h2>{local_text('What is included?', '收录什么数据？')}</h2><p>{local_text('Public bacterial transcript 3′-end coordinates derived from experiments and retained with the definitions used by each source paper.', '收录来自公开实验的细菌转录本 3′ 端坐标，并保留各来源论文对这些端点的原始定义。')}</p></div></article>
    <article><span class="intro-number">02</span><div><h2>{local_text('What does one record mean?', '一条记录代表什么？')}</h2><p>{local_text('One source-specific 3′-end position on an exact reference sequence, with its strand, evidence class, and study provenance.', '一条记录是在精确参考序列上的一个来源特异性 3′ 端位置，并带有链方向、证据类别和研究来源。')}</p></div></article>
    <article><span class="intro-number">03</span><div><h2>{local_text('Important evidence boundary', '重要的证据边界')}</h2><p>{local_text('A detected 3′ end is not automatically a functionally validated transcription terminator; it may also reflect RNA processing.', '检测到的 3′ 端不自动等同于经过功能验证的转录终止子，也可能反映 RNA 加工。')}</p></div></article>
  </section>
  ...
  <section data-edge-results hidden>
    ...
    <section class="panel edge-studies"><div class="panel-header"><div><p class="eyebrow">{local_text('Experimental datasets', '实验数据集')}</p><h2>{local_text('Studies available for this genome', '该基因组的可用研究')}</h2></div><span class="browser-guide-hint">{local_text('Studies remain separate tracks', '各研究保留为独立轨道')}</span></div><p class="section-note">{local_text('Rows from different papers are not merged into a consensus. Open the genome browser to compare them on the same reference coordinates.', '不同论文的记录不会合并为共识结果；可在同一参考坐标上打开基因组浏览器进行比较。')}</p><div class="table-wrap"><table class="source-table edge-study-table"><thead><tr><th>{local_text('Study and publication', '研究与文献')}</th><th>{local_text('Assay', '实验方法')}</th><th>{local_text('Evidence', '证据')}</th><th>{local_text('Records', '记录数')}</th><th>{local_text('Source links', '来源链接')}</th></tr></thead><tbody data-edge-studies></tbody></table></div></section>
    <section class="panel edge-interpretation"><div><p class="eyebrow">{local_text('Interpretation', '数据解读')}</p><h2>{local_text('What do these records represent?', '这些记录具体代表什么？')}</h2><p>{local_text('Both studies map their reported 3′ ends to the same reference assembly, so the coordinates can be viewed together. Their endpoint definitions and source identities remain independent.', '两项研究均把报告的 3′ 端定位到同一参考组装，因此坐标可以放在一起查看，但端点定义和来源身份仍相互独立。')}</p><ul class="interpretation-list" data-edge-interpretations></ul></div><aside><h3>{local_text('Suitable uses', '适合的用途')}</h3><ul><li>{local_text('Inspect endpoints relative to nearby genes', '查看端点与邻近基因的关系')}</li><li>{local_text('Compare agreement between independent studies', '比较独立研究之间的一致性')}</li><li>{local_text('Export coordinates for downstream analysis', '导出坐标进行下游分析')}</li></ul><p><strong>{local_text('Do not infer:', '不能直接推断：')}</strong> {local_text('that every record is a functional terminator.', '每条记录都是功能性终止子。')}</p></aside></section>
