...
|---|---|
| 发布判定 | `to_review` |
| 公开资产 | `data/records/BTED_EXT_2026_103/`（24 列端点表 + BED6） |
| 记录数 | 见端点表行数（`BTED_EXT_2026_103_endpoints.tsv`） |
...
- 端点坐标：`1-based 单碱基位点 (2026-08-09 与 Table S2 交叉验证确认): 本表端点=Table S1 gene_peak_posn(defined end 的 3' 峰位), S1 无 U_tract 类序列列故无法直接序列验证, 改用交叉比对法——将 S1 gene_peak_posn 与 S2 已序列实证的 peak_position (S2 388 行 U_tract 100% 吻合 CP000100.1 序列)…`
