...
| `data_verification_report.md` | 逐篇核查 README 声明与已下载文件的对应关系；13/13 来源均确认存在包含坐标字段的核心补充表，行数核对总体相符，但部分来源存在筛选口径、混合表内容或小幅行数差异，尚不能据此将表内每条记录统一视为实验验证终点 | 坐标数据验证声明 |
...
> 注：上表为对各文档既有内容的客观描述。按评审意见收敛表述：A 类补充表**可作为后续标准化和逐记录证据审核的候选输入**；这不代表坐标体系已标准化、参考基因组版本已确认或表内每条记录均为实验验证终点，也不意味着相关内容可不经证据分层直接用于数据库构建。
...
- 当前 git pack 约 30.72 MiB；`文献13` 已追踪的 read-starts 文本文件（未压缩合计约 167.97 MiB）对压缩后 pack 大小的具体贡献未单独核实。
...
| 候选材料 | 权威来源 | 预期大小 | 公开适用性 | 迁移风险 |
|----------|----------|----------|------------|----------|
| 22 来源注册表 | 外部 BTED 工作树（`to verify`） | KB | 审阅后适用 | **必须解释数量口径差异**：需要区分三个数字 —— 本仓库核实的 13 篇 PMID、BATTER Table S1 在这 13 篇 PMID 下列出的 22 条记录、外部工作树据报的 22 来源注册表。外部注册表与 Table S1 的 22 条记录是否一一对应尚未核实（`to verify`），不得默认二者相同；注册表的单位（文献 / 记录 / 物种 × 条件数据集）也未确定 |
