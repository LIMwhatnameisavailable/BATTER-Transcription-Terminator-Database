...
若一个分支已经包含多篇外部文献的来源登记、初步坐标核验或临时调研笔记，**不要直接整体合并**。请按[外部来源正式整合入库要求 v0.1](docs/standards/外部来源正式整合入库要求_v0.1.md)建立交接快照和批次整合说明，再由维护者从当前公共基线创建整合分支，选择性接收可复现材料。

- 来源登记快照进入 `data/registry/submissions/`；
- 批次整合决定进入 `docs/integration/`；
- 每个来源仍须独立完成处理记录和端点标准化后，才能进入 `standardized` / `published`；
- 已发表算法重分析的实验端点、作者直接发表端点与预测结果必须分别标识；新枚举值须先经团队确认。
